...
class PostFilters
{
    ...
    public function fltAllowAutodraftEditing($exception_clause, $required_operation, $post_type, $args)
    {
        if (('edit' == $required_operation) && ('include' == $args['mod']) && is_admin()) {
            ...
            if (in_array($pagenow, ['post-new.php', 'post.php']) && !PWP::empty_POST() && PWP::is_REQUEST('action', ['edit', 'editpost'])) {
                if ($post_id = PWP::getPostID()) {
                    ...
                    if (
                        ($current_user->ID == $_post->post_author) && ($_post->post_type == $post_type)
    ...
    public function fltApplyListAllPosts($missing_caps, $reqd_caps, $post_type, $meta_cap)
    {
